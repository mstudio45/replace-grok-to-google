...
(function() {
    const CONFIG = {
        ...
        translateTo: GM_getValue("translateTo", "en"),
        createMissingTranslateBtn: GM_getValue("createMissingTranslateBtn", false)
    };
    ...
    GM_registerMenuCommand(`[${CONFIG.translateTo}] Target Language`, () => {
        const newLang = prompt("Enter target language code (e.g., en, es, fr, ja):", CONFIG.translateTo);
        if (newLang !== null) updateValue("translateTo", newLang.trim().toLowerCase() || "en");
    });

    GM_registerMenuCommand(`[${CONFIG.createMissingTranslateBtn ? "✅" : "❌"}] Create missing translate button for tweets`, () => {
        updateValue("createMissingTranslateBtn", !CONFIG.createMissingTranslateBtn);
    });

    GM_registerMenuCommand(`[${CONFIG.debug ? "✅" : "❌"}] Debug Logs`, () => {
        updateValue("debug", !CONFIG.debug);
    });
    ...
    GM_addStyle(`
        ...
        .tweet-btn-text-outer {
            color: rgb(29, 155, 240);
            display: inline-flex;
            align-items: center;
        }

        .tweet-btn-text-inner {
            white-space: nowrap;
            overflow: hidden;
    ...
    const translateButtonDiv = `
        <div class="tweet-btn-container">
            ${googleTranslateSvg}
            <button aria-label="Show translation" role="button" class="tweet-btn-button" type="button">
                <span class="tweet-btn-text-outer">
                    <span class="tweet-btn-text-inner">
    ...
    const buttonObserver = new MutationObserver(() => {
        document.querySelectorAll('button[aria-label="Show translation"]').forEach((btn) => {
            ...
            btn.addEventListener("click", async (event) => {
                ...
                const tweetContainer = btn.parentElement.parentElement; // btn.closest("article");
                ...
                try {
                    const translatedText = await translateText(originalText, CONFIG.translateTo);
    ...
    // tweet handler //
    if (CONFIG.createMissingTranslateBtn) {
        const tweetObserver = new MutationObserver(() => {
            document.querySelectorAll('div[data-testid="tweetText"][lang]:not([lang^="' + CONFIG.translateTo + '"])').forEach((tweet) => {
                if (tweet.dataset.translateBtnProcessed) return;
                tweet.dataset.translateBtnProcessed = "true";

                const container = tweet.parentElement;
                if (!container) return;

                const hasCustom = container.querySelector(".tweet-btn-container");
                const hasNative = container.querySelector('button[aria-label="Show translation"]');
                if (hasCustom || hasNative) return;
    
                const text = extractTweetText(tweet);
                if (!isTranslatable(text)) return;

                debugLog("Adding translation button to tweet...");
                container.insertAdjacentHTML("afterbegin", translateButtonDiv);
            });
        });
